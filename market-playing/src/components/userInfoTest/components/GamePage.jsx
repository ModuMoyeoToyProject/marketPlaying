    ...
    return (
        ...
        > 
            <div className={"controlArea "+gameMode}>
                <div className="pdd10"> 
                    <button className="btn" onClick={restart()}>{"초기화"}</button>
                </div>
                <div className="pdd10">
                ...
                <div className="pdd10">
                    <button className="btn" onClick={addTime(720)}>{"+12시간 낮 밤 변경"}</button>
                    <button className="btn" onClick={addTime(30)}>{"+30분"}</button>
                    <button className="btn" onClick={addTime(-10)}>{"-10분"}</button>
                </div>
                <div className="pdd10">
                    {"Wallet > "} {userWallet}
                </div>
                <div className="pdd10">
                    <button className="btn" onClick={changeAmount(100)}>{"+100원"}</button>
                    <button className="btn" onClick={changeAmount(500)}>{"+500원"}</button>
                    <button className="btn" onClick={changeAmount(1000)}>{"+1000원"}</button>
                </div>
                <div className="pdd10">
                    <button className="btn" onClick={changeAmount(-100)}>{"-100원"}</button>
                    <button className="btn" onClick={changeAmount(-500)}>{"-500원"}</button>
