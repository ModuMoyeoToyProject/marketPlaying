...
const GamePage = ({
    ...
    <div style={{justifyContent: "flex-start", flexDirection: "row", display: "flex"}}>
        ...
        <div className="userArea rdBox1" >
            ...
            <button className="span" onClick={fn_addTime(720)}>{"+12시간(낮<->밤)"}</button>
        </div>
        <div className="userArea rdBox1" >
        ...
        <div className="userArea rdBox1" >
            ...
            <button className="span" onClick={fn_move(preMenu)}>{"돌아가기"}</button>
        </div> 
    </div>
    ...
    <div style={{justifyContent: "flex-start", flexDirection: "row", display: "flex"}}>
        ...
        <div className="userArea ">
            <button className="span" onClick={fn_addTime(10)}>{"+10분"}</button>
            <button className="span" onClick={fn_addTime(-10)}>{"-10분"}</button>
        </div>
        <div className="userArea ">
        <button className="span" onClick={fn_updateBag({}, 1000, 1)}>{"-1000"}</button>
        <button className="span" onClick={fn_updateBag({}, 1000, -1)}>{"+1000"}</button>
        <button className="span" onClick={fn_updateBag({}, 0, 0)}>{"0"}</button>
        </div> 
        <div className="userArea ">
        <button className="span" onClick={fn_move("lake")}>{"호수"}</button>
        <button className="span" onClick={fn_move("shop")}>{"상점"}</button>
        <button className="span" onClick={fn_move("street")}>{"거리"}</button>
        </div> 
    </div>
    </div> 
    );
